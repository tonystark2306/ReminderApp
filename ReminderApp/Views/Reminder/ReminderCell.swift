...
class ReminderCell: UITableViewCell, UITextViewDelegate {
    ...
    func configure(title: String?, note: String?, useHints: Bool) {
        if let t = title, !t.isEmpty { titleTextView.text = t; isHintTitle = false }
        else if useHints { titleTextView.text = hintTitle; isHintTitle = true }
        else { titleTextView.text = ""; isHintTitle = false }

        if let n = note, !n.isEmpty { noteTextView.text = n; isHintNote = false }
        else if useHints { noteTextView.text = hintNote; isHintNote = true }
        else { noteTextView.text = ""; isHintNote = false }
    }

    func textViewDidBeginEditing(_ textView: UITextView) {
        infoButton.isHidden = false
        if textView === titleTextView, isHintTitle { isHintTitle = false; titleTextView.text = "" }
        else if textView === noteTextView, isHintNote { isHintNote = false; noteTextView.text = "" }
    }
    ...
    @objc private func tapInfo() { onTapInfo?() }
    @objc private func tapDone() { onTapDone?() }   
}
